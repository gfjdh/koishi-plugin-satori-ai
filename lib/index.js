...
var SAT = class extends Sat {
  ...
  registerCommands(ctx) {
    ...
    ctx.command("sat.group_sense <text:text>", "添加群常识").action(async ({ session }, prompt) => this.addGroupCommonSense(session, prompt));
...
`;
  ...
  // 添加群常识
  async addGroupCommonSense(session, content) {
    if (!content)
      return session.text("commands.sat.common_sense.messages.no-prompt");
    const filePath = path6.join(this.config.dataDir, `group_sense`, `${session.channelId}.txt`);
    await this.memoryManager.saveLongTermMemory(session, [{
      role: "user",
      content: content.replace(/\"/g, "'").trim()
    }], filePath);
