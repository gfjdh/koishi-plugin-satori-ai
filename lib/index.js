...
async function wrapInHTML(str, width = 20) {
  ...
  try {
    ...
    const timeoutPromise = new Promise(
      (_, reject) => setTimeout(() => reject(new Error("Puppeteer 渲染超时")), 3e4)
    );
...
// src/emoji.ts
var logger2 = new import_koishi2.Logger("satori-emoji");
var EmojiManager = class {
  constructor(ctx, config) {
    this.ctx = ctx;
    this.config = config;
    this.emojiDir = path2.resolve(this.config.dataDir, "emojis");
    this.loadEmojis();
  }
  static {
    __name(this, "EmojiManager");
  }
  categories = [];
  emojiDir;
  async loadEmojis() {
    try {
      if (!fs2.existsSync(this.emojiDir)) {
        logger2.warn(`表情包目录不存在: ${this.emojiDir}`);
        return;
      }
      const entries = await fs2.promises.readdir(this.emojiDir, { withFileTypes: true });
      const folders = entries.filter((entry) => entry.isDirectory());
      for (const folder of folders) {
        const folderPath = path2.join(this.emojiDir, folder.name);
      ...
      logger2.info(`已加载表情包类别: ${this.categories.map((c) => c.name).join(", ")}`);
    } catch (error) {
      logger2.error(`加载表情包失败: ${error}`);
    }
  }
  shouldSendEmoji() {
    if (!this.config.enable_emoji)
      return false;
    return Math.random() < this.config.emoji_probability;
  }
  async getEmoji(session, user, recentMessages) {
...
${fullPrompt}`);
    ...
    try {
      ...
      logger2.info(`模型推荐的表情包类别: ${categoryName}`);
      if (!categoryName || categoryName === "none")
        return null;
      const category = this.categories.find((c) => c.name === categoryName);
      if (category) {
        const randomFile = category.files[Math.floor(Math.random() * category.files.length)];
        const filePath = path2.join(this.emojiDir, category.name, randomFile);
        logger2.info(`选择表情包: ${category.name}/${randomFile}`);
        return filePath;
      } else {
        logger2.info(`模型返回了无效的表情包类别: ${categoryName}`);
        return null;
      }
    } catch (error) {
      logger2.warn(`获取表情包失败: ${error}`);
      return null;
    }
  }
  getAPIConfig(user) {
    ...
    return {
      ...
      key: this.config.not_reasoner_LLM_key[0],
      // 简化处理，使用第一个key
      model: this.config.not_reasoner_LLM
    };
  }
  async callLLM(baseURL, key, model, messages) {
    ...
    try {
      const response = await this.ctx.http.post(url, payload, { headers, timeout: 1e4 });
      logger2.info(`LLM响应: ${JSON.stringify(response)}`);
      ...
      if (isErrorWithMessage(error)) {
        ...
        logger2.warn(`API Error [${status}]: ${message}`);
      }
      throw error;
    }
  }
...
var logger3 = new import_koishi3.Logger("satori-api");
var APIClient = class {
  constructor(ctx, config) {
    this.ctx = ctx;
    this.config = config;
  ...
  async auxiliaryChat(messages) {
    ...
    for (let i = 1; i <= this.config.maxRetryTimes; i++) {
      try {
        ...
        logger3.warn(`辅助模型API请求失败(${error})，重试(第${i}次)中...`);
        continue;
      }
    }
  ...
  async generateUserPortrait(user, messages) {
    ...
    for (let i = 1; i <= this.config.maxRetryTimes; i++) {
      try {
        ...
        await new Promise((resolve4) => setTimeout(resolve4, this.config.retry_delay_time || 5e3));
        logger3.warn(`生成画像时API请求失败(${error})，重试(第${i}次)中...`);
        continue;
      }
    }
    return { content: "unreachable", error: true };
  ...
  // 生成请求体
  createPayload(messages, model, thinkingType = "enabled") {
    return {
      model,
      messages,
  ...
  async tryRequest(URL, payload, keys) {
    ...
    for (let i = 1; i <= this.config.maxRetryTimes; i++) {
      try {
        const response = await this.ctx.http.post(url, payload, { headers, timeout: 36e5 });
        logger3.info(`APIID: ${response.id}, 输入token: ${response.usage.prompt_tokens}, 输出token: ${response.usage.completion_tokens}, 总token: ${response.usage.total_tokens}`);
        content = response.choices[0].message.content;
        const reasoning_content = response.choices[0].message.reasoning_content || "无";
        if (this.config.reasoning_content)
          logger3.info(`思维链: ${reasoning_content}`);
        if (!content && reasoning_content) {
          logger3.warn("返回内容为空,但存在推理内容");
        }
        if (reasoning_content != "无" && content && content.length > reasoning_content.length) {
          logger3.warn("返回内容疑似包含推理内容");
        }
        if (response.usage.completion_tokens > this.config.max_output_tokens) {
          logger3.warn("返回内容过长导致被截断");
          continue;
        }
        ...
        await new Promise((resolve4) => setTimeout(resolve4, this.config.retry_delay_time || 5e3));
        logger3.warn(`API请求失败(${error})，重试(第${i}次)中...`);
        continue;
      }
    }
  ...
  handleAPIError(error) {
    ...
    const message = error.response?.data?.error?.message || error.message;
    logger3.error(`API Error [${status}]: ${errorCode} - ${message}`);
    switch (status) {
      case 400:
  ...
  rotateKey() {
    ...
    logger3.debug(`Switched to API key index: ${this.currentKeyIndex}`);
  }
  // 测试连接
  async testConnection() {
    try {
      await this.ctx.http.get(`${trimSlash(this.config.baseURL)}/models`, { headers: this.createHeaders(this.config.keys) });
      logger3.info("API connection test succeeded");
      return true;
    } catch (error) {
      logger3.warn("API connection test failed");
      return false;
    }
  }
};
...
var logger4 = new import_koishi4.Logger("satori-memory");
var MemoryManager = class {
  ...
  async saveLongTermMemory(session, dialogues, filePath = "") {
    ...
    await fs3.promises.writeFile(filePath, JSON.stringify(updated, null, 2), "utf-8");
  }
  // 记忆检索
  async searchMemories(session, type = "user") {
    const filePathMap = {
      "user": this.getUserMemoryPath(session.userId),
      "common": path3.join(this.config.dataDir, "common_sense.txt"),
      "group": path3.join(this.config.dataDir, "group_sense", `${session.channelId}.txt`)
    };
    const topNMap = {
    ...
    const userDialogues = recentMemories.filter((entry) => entry.role === "user").map((entry) => entry.content).join("");
    ...
    if (type === "user") {
      ...
      await fs3.promises.writeFile(filePath, JSON.stringify(updatedEntries, null, 2), "utf-8");
    }
...
`;
  ...
  getUserMemoryPath(userId) {
    return path3.join(this.config.dataDir, "dialogues", `${userId}.txt`);
  }
  // 确保记忆文件存在
  async ensureMemoryFile(filePath) {
    await fs3.promises.mkdir(path3.dirname(filePath), { recursive: true });
    if (!fs3.existsSync(filePath)) {
      await fs3.promises.writeFile(filePath, "[]", "utf-8");
    }
  }
  // 加载记忆文件
...
async function handleFixedDialogues(ctx, session, user, prompt, config) {
  if (!config.enable_fixed_dialogues)
    return null;
  const filePath = path4.join(config.dataDir, "fixed_dialogues.json");
...
async function ensureFixedDialoguesFile(filePath) {
  if (!fs4.existsSync(filePath)) {
    ...
    fs4.writeFileSync(filePath, JSON.stringify(defaultDialogues, null, 2));
  }
}
...
async function outputContentCheck(ctx, response, userid, config, session, moodManager) {
  ...
  if (config.output_censor_favorability) {
    ...
    const filePath = path5.join(config.dataDir, "output_censor.txt");
    if (!fs5.existsSync(filePath)) {
      return 0;
    }
    const censorWords = fs5.readFileSync(filePath, "utf-8").split(",");
...
async function ensureCensorFileExists(basePath) {
  const filePath = path5.join(basePath, "output_censor.txt");
  fs5.mkdirSync(path5.dirname(filePath), { recursive: true });
  if (!fs5.existsSync(filePath)) {
    fs5.writeFileSync(filePath, "示例屏蔽词1,示例屏蔽词2,示例屏蔽词3", "utf-8");
  }
}
...
var logger5 = new import_koishi5.Logger("satori-ai-middleware");
...
var Sat = class extends import_koishi6.Service {
  static {
    __name(this, "Sat");
  }
  static inject = {
...
((Sat2) => {
  Sat2.Config = import_koishi6.Schema.intersect([
    import_koishi6.Schema.object({
      baseURL: import_koishi6.Schema.string().default("https://api.deepseek.com").description("深度思考模型请求地址"),
      key: import_koishi6.Schema.union([
        import_koishi6.Schema.array(String).role("secret"),
        import_koishi6.Schema.transform(String, (value) => [value])
      ]).default([]).role("secret").description("深度思考模型api_key"),
      appointModel: import_koishi6.Schema.string().default("deepseek-reasoner").description("深度思考模型"),
      enhanceReasoningProtection: import_koishi6.Schema.boolean().default(true).description("是否增强思维链保护（开启后会强制输出内容带有限制标签，极大防止思维链溢出问题）"),
      not_reasoner_LLM_URL: import_koishi6.Schema.string().default("https://api.deepseek.com").description("非深度思考模型请求地址"),
      not_reasoner_LLM: import_koishi6.Schema.string().default("deepseek-chat").description("非深度思考模型(用于节省成本"),
      not_reasoner_LLM_key: import_koishi6.Schema.union([
        import_koishi6.Schema.array(String).role("secret"),
        import_koishi6.Schema.transform(String, (value) => [value])
      ]).default([]).role("secret").description("非深度思考模型api_key"),
      use_not_reasoner_LLM_length: import_koishi6.Schema.number().default(8).description("触发使用非深度思考模型的字数（输入少于此字数时）,如果你希望始终使用非深度思考模型进行对话，请将此值设置成一个较大的值,反之亦然"),
      enable_reasoner_like: import_koishi6.Schema.boolean().default(true).description("是否启用非深度思考模型模仿思维链（可以在节省成本的同时提高效果，但是模型经常不按规定格式输出，所以不推荐开启）"),
      auxiliary_LLM_URL: import_koishi6.Schema.string().default("https://api.deepseek.com").description("辅助模型请求地址"),
      auxiliary_LLM: import_koishi6.Schema.string().default("deepseek-chat").description("辅助模型(用于好感度调整等功能，如不需要可不填，建议使用低成本模型"),
      auxiliary_LLM_key: import_koishi6.Schema.union([
        import_koishi6.Schema.array(String).role("secret"),
        import_koishi6.Schema.transform(String, (value) => [value])
      ]).default([]).role("secret").description("辅助模型api_key"),
      prompt: import_koishi6.Schema.string().role("textarea").description("人格设定"),
      reasoner_prompt: import_koishi6.Schema.string().role("textarea").description("思考提示词（对于非深度思考模型模仿思维链时生效）").default("你在思考时应当逐步分析以下内容：1.当前对话的禁止事项；2.是否与群聊中其他人的发言有关联；3.是否与当前用户之前说的话有关联；4.对话的具体要求；5.用户的意图如何；6.最终的发言的主要内容。")
    }).description("基础设置"),
    import_koishi6.Schema.object({
      alias: import_koishi6.Schema.array(String).default(["ai"]).description("触发命令;别名"),
      authority: import_koishi6.Schema.number().role("slider").min(0).max(5).step(1).description("允许使用的最低权限").default(1),
      no_system_prompt: import_koishi6.Schema.boolean().default(false).description("是否将系统提示的system替换为user（用于具有思维链的模型）"),
      max_tokens: import_koishi6.Schema.number().description("最大请求长度（字符数）").default(100),
      max_output_tokens: import_koishi6.Schema.number().description("最大返回长度(不含思维链)（token数）").default(4096),
      message_max_length: import_koishi6.Schema.number().description("最大频道上下文长度（条数）").default(10),
      temperature: import_koishi6.Schema.number().role("slider").min(0).max(2).step(0.01).default(0.5).description("温度"),
      frequency_penalty: import_koishi6.Schema.number().default(0).description("频率惩罚"),
      presence_penalty: import_koishi6.Schema.number().default(0).description("存在惩罚"),
      maxRetryTimes: import_koishi6.Schema.number().default(10).description("报错后最大重试次数"),
      retry_delay_time: import_koishi6.Schema.number().default(5e3).description("每次重试延迟时间"),
      max_parallel_count: import_koishi6.Schema.number().default(2).description("频道最大并行请求数"),
      log_system_prompt: import_koishi6.Schema.boolean().default(false).description("是否在日志中输出系统提示"),
      log_reasoning_content: import_koishi6.Schema.boolean().default(true).description("是否在日志中输出思维链"),
      log_ask_response: import_koishi6.Schema.boolean().default(true).description("是否在日志中输出问答")
    }).description("请求设置"),
    import_koishi6.Schema.object({
      enable_self_memory: import_koishi6.Schema.boolean().default(true).description("是否启用模型自发言记忆（仅短期）"),
      personal_memory: import_koishi6.Schema.boolean().default(true).description("是否启用按人记忆（否则将群内所有人视为同一个用户）"),
      channel_dialogues: import_koishi6.Schema.boolean().default(true).description("是否获取群聊内最近对话（包括不对bot说的）"),
      channel_dialogues_max_length: import_koishi6.Schema.number().default(20).description("群聊内最近对话最大长度(条数)"),
      bracket_filter: import_koishi6.Schema.boolean().default(false).description("是否启用括号过滤，开启后在写入短期记忆时会过滤掉括号内的内容，用于缓解复读问题"),
      memory_filter: import_koishi6.Schema.string().role("textarea").default("示例1-示例2").description("短期记忆过滤词，使用“-”分隔，含有过滤词的那一句不会被记忆，用于缓解复读问题"),
      dataDir: import_koishi6.Schema.string().default("./data/satori_ai").description("聊天记录保存位置（长期记忆）"),
      memory_block_words: import_koishi6.Schema.array(String).default(["好感"]).description("记忆屏蔽词"),
      remember_min_length: import_koishi6.Schema.number().description("触发保存到记忆的长度").default(20),
      common_topN: import_koishi6.Schema.number().default(5).description("群常识、常识记忆检索最大匹配数"),
      dailogues_topN: import_koishi6.Schema.number().default(5).description("对话记忆检索最大匹配数"),
      enable_fixed_dialogues: import_koishi6.Schema.boolean().default(false).description("是否启用固定对话（在dataDir中的fixed_dialogues.json修改）")
    }).description("记忆设置"),
    import_koishi6.Schema.object({
      max_usage: import_koishi6.Schema.tuple([Number, Number, Number, Number, Number]).default([40, 240, 3e3, 9999, 0]).description("每日最大使用次数(对应用户level0~level4)(0为不限制)"),
      private: import_koishi6.Schema.boolean().default(false).description("开启后私聊AI可触发对话, 不需要使用指令"),
      nick_name: import_koishi6.Schema.boolean().default(true).description("是否使用昵称触发对话（发言中含有昵称时）（把qq号加入昵称可以被at和戳一戳触发）"),
      nick_name_list: import_koishi6.Schema.array(String).default(["昵称1"]).description("昵称列表"),
      nick_name_block_words: import_koishi6.Schema.array(String).default(["屏蔽词1"]).description("昵称屏蔽词(含有屏蔽词的消息不会触发昵称对话)"),
      reasoner_filter: import_koishi6.Schema.boolean().default(true).description("是否启用返回内容过滤,开启后在对话时会过滤掉在括号内且含有过滤词的那一句,用于缓解思维链溢出问题"),
      reasoner_filter_word: import_koishi6.Schema.string().role("textarea").default("系统-提示-用户-设定-回复").description("返回内容过滤词，使用“-”分隔，在括号内且含有过滤词的那一句会被过滤，用于缓解思维链溢出问题"),
      duplicateDialogueCheck: import_koishi6.Schema.boolean().default(true).description("是否检查重复对话"),
      enable_online_user_check: import_koishi6.Schema.boolean().default(true).description("在未回答而再次提问时是否提示用户有未完成的对话"),
      random_min_tokens: import_koishi6.Schema.number().default(20).description("随机触发对话的最小长度"),
      enable_random_without_favorability: import_koishi6.Schema.boolean().default(true).description("是否启用随机触发对话无视高好感度（即使用户好感度很高也会最多视为朋友）"),
      randnum: import_koishi6.Schema.number().role("slider").min(0).max(1).step(0.01).default(0).description("在群聊中随机触发对话的概率，如需关闭可设置为 0"),
      sentences_divide: import_koishi6.Schema.boolean().default(true).description("是否分句发送"),
      min_sentences_length: import_koishi6.Schema.number().default(10).description("每个分句的最小长度"),
      max_sentences_length: import_koishi6.Schema.number().default(20).description("每个分句的最大长度"),
      time_interval: import_koishi6.Schema.number().default(1e3).description("每句话的时间间隔"),
      reply_pointing: import_koishi6.Schema.boolean().default(true).description("是否在与多人同时对话时显示回复指向"),
      enable_emoji: import_koishi6.Schema.boolean().default(false).description("是否启用表情包功能（表情包文件按类命名分成文件夹放在数据路径下的emojis文件夹，最好是gif）"),
      emoji_probability: import_koishi6.Schema.number().role("slider").min(0).max(1).step(0.01).default(0.1).description("在对话中发送表情包的概率")
    }).description("对话设置"),
    import_koishi6.Schema.object({
      enable_weather_perception: import_koishi6.Schema.boolean().default(false).description("是否开启天气感知（JWT 配置可选，若同时配置私钥/kid/sub 则优先使用 JWT）"),
      Location_search_API_URL: import_koishi6.Schema.string().default("https://api.example.com/location/search.json?").description("位置搜索API地址（请至少填写到“?”）"),
      location_param_name: import_koishi6.Schema.string().default("location").description("位置参数名称"),
      weather_api_URL: import_koishi6.Schema.string().default("https://api.example.com/weather/now.json?").description("天气API地址（请至少填写到“?”）"),
      weather_param_name: import_koishi6.Schema.string().default("location").description("天气位置参数名称"),
      weather_api_key: import_koishi6.Schema.string().role("secret").default("").description("天气API Key"),
      weather_cd_time: import_koishi6.Schema.number().default(60).description("天气更新请求的最小间隔（分钟）（在对话时检查更新）"),
      // JWT 配置（可选，若同时配置私钥/kid/sub 则优先使用 JWT）
      weather_jwt_private_key_pem: import_koishi6.Schema.string().role("secret").default("").description("Ed25519 私钥 PEM（包含 -----BEGIN PRIVATE KEY----- 和 -----END PRIVATE KEY-----）"),
      weather_jwt_kid: import_koishi6.Schema.string().default("").description("JWT 凭据 ID (kid)，来自控制台凭据页面"),
      weather_jwt_sub: import_koishi6.Schema.string().default("").description("JWT 的 sub（项目 ID）"),
      weather_jwt_exp_seconds: import_koishi6.Schema.number().default(300).description("JWT 有效期（秒），最大 86400（24 小时）")
    }).description("天气感知设置"),
    import_koishi6.Schema.object({
      enable_favorability: import_koishi6.Schema.boolean().default(false).description("是否开启好感度系统(每次对话默认+1好感度)"),
      max_favorability_perday: import_koishi6.Schema.number().default(100).description("每日有效(引发好感度增长)对话次数上限"),
      input_censor_favorability: import_koishi6.Schema.boolean().default(false).description("是否开启好感度审查(通过输入屏蔽词扣除好感)"),
      value_of_input_favorability: import_koishi6.Schema.number().default(15).description("输入触发输入屏蔽词每次扣除的好感度"),
      output_censor_favorability: import_koishi6.Schema.boolean().default(false).description("通过输出屏蔽词扣除好感,在dataDir中的output_censor.txt修改)"),
      value_of_output_favorability: import_koishi6.Schema.number().default(15).description("输出触发输出屏蔽词每次扣除的好感度"),
      enable_auxiliary_LLM: import_koishi6.Schema.boolean().default(false).description("是否使用辅助大模型判断好感度增减(量与输入屏蔽词每次扣除的好感度相关,不稳定，慎用)"),
      offset_of_fafavorability: import_koishi6.Schema.number().default(3.5).description("辅助大模型好感度偏移量(越大越容易扣好感度)"),
      visible_favorability: import_koishi6.Schema.boolean().default(true).description("是否开启好感度升降显示"),
      enable_user_portrait: import_koishi6.Schema.boolean().default(true).description("是否启用用户画像功能"),
      portrait_usage: import_koishi6.Schema.number().default(2).description("每天触发第几次对话后触发画像生成（最小为2）"),
      max_portrait_dialogues: import_koishi6.Schema.number().default(100).description("画像生成获取的最大对话条数"),
      portrait_min_favorability: import_koishi6.Schema.number().default(100).description("触发画像生成的最小好感度"),
      prompt_0: import_koishi6.Schema.string().role("textarea").description("厌恶好感补充设定"),
      favorability_div_1: import_koishi6.Schema.number().default(15).description("厌恶-陌生分界线"),
      prompt_1: import_koishi6.Schema.string().role("textarea").description("陌生好感补充设定"),
      favorability_div_2: import_koishi6.Schema.number().default(150).description("陌生-朋友分界线"),
      prompt_2: import_koishi6.Schema.string().role("textarea").description("朋友好感补充设定"),
      favorability_div_3: import_koishi6.Schema.number().default(500).description("朋友-思慕分界线"),
      prompt_3: import_koishi6.Schema.string().role("textarea").description("思慕好感补充设定"),
      favorability_div_4: import_koishi6.Schema.number().default(1e3).description("思慕-恋慕分界线"),
      prompt_4: import_koishi6.Schema.string().role("textarea").description("恋慕好感补充设定"),
      prompt_5: import_koishi6.Schema.string().role("textarea").default("一般此配置项无效").description("夫妻好感补充设定"),
      enable_warning: import_koishi6.Schema.boolean().default(false).description("是否开启打小报告（当有人好感度下降时会在主群里打小报告）"),
      warning_group: import_koishi6.Schema.string().default("").description("打小报告的群号"),
      warning_admin_id: import_koishi6.Schema.string().default("").description("打小报告艾特的管理员id")
    }).description("好感度设置"),
    import_koishi6.Schema.object({
      enable_mood: import_koishi6.Schema.boolean().default(false).description("是否开启心情系统（通过屏蔽词降低心情，正常聊天心情+1，每天重置为mood_div_1 + value_of_output_mood）"),
      max_mood: import_koishi6.Schema.number().default(10).description("心情上限"),
      value_of_input_mood: import_koishi6.Schema.number().default(10).description("输入触发屏蔽词每次扣除的心情"),
      value_of_output_mood: import_koishi6.Schema.number().default(5).description("输出触发屏蔽词每次扣除的心情,若当时心情大于0,此次不扣好感度,会根据好感度等级调整数值"),
      visible_mood: import_koishi6.Schema.boolean().default(false).description("是否开启心情状态显示"),
      mood_prompt_2: import_koishi6.Schema.string().role("textarea").default("你现在的心情十分愉悦").description("心情达到最高补充设定"),
      mood_div_1: import_koishi6.Schema.number().default(-1).description("心情正常-烦躁分界线"),
      mood_prompt_0: import_koishi6.Schema.string().role("textarea").default("你现在的心情是：有点烦躁").description("烦躁心情补充设定"),
      mood_div_2: import_koishi6.Schema.number().default(-15).description("烦躁-生气分界线"),
      mood_prompt_1: import_koishi6.Schema.string().role("textarea").default("你现在的心情是：非常生气").description("生气心情补充设定"),
      enable_pocket_money: import_koishi6.Schema.boolean().default(false).description("开启要零花钱指令（通过消耗心情换随机数量p点）"),
      min_pocket_money: import_koishi6.Schema.number().default(100).description("随机p点最小值"),
      max_pocket_money: import_koishi6.Schema.number().default(400).description("随机p点最大值"),
      pocket_money_cost: import_koishi6.Schema.number().default(2).description("消耗心情换p点的心情值")
    }).description("好感度拓展：心情设置（需要开启好感度模块才能生效）"),
    import_koishi6.Schema.object({
      enable_galgame: import_koishi6.Schema.boolean().default(true).description("是否启用 galgame 模块")
    }).description("galgame 设置，详细功能请开启后进行默认事件体验"),
    import_koishi6.Schema.object({
      Broadcast_block_channel: import_koishi6.Schema.array(String).description("不提供广播的频道id"),
      waiting_time: import_koishi6.Schema.number().default(1440).description("广播在缓存中等待的最大时间（分钟）（超过时间后没有触发会不再推送）")
    }).description("广播设置（发出后在每个生效群收到首条消息时会推送）"),
    import_koishi6.Schema.object({
      enable_game: import_koishi6.Schema.boolean().default(false).description("是否开启游戏模块"),
      game_block_channel: import_koishi6.Schema.array(String).description("不提供游戏的频道id"),
      enable_OneTouch: import_koishi6.Schema.boolean().default(false).description("是否开启一碰一游戏（需要puppeteer支持）"),
      cd_for_OneTouch: import_koishi6.Schema.number().default(600).description("每人一碰一游戏cd时间（秒）"),
      enable_fencing: import_koishi6.Schema.boolean().default(false).description("是否开启击剑游戏(无效，开发中)")
...
var logger6 = new import_koishi7.Logger("satori-portrait");
var UserPortraitManager = class {
  ...
  getPortraitPath(userId) {
    return path6.join(this.config.dataDir, "UserPortrait", `${userId}.txt`);
  }
  // 检查用户印象文件
  hasPortrait(userId) {
    return fs6.existsSync(this.getPortraitPath(userId));
  }
  // 读取用户印象
  readPortrait(userId) {
    try {
      return fs6.readFileSync(this.getPortraitPath(userId), "utf-8");
    } catch {
      return "";
    }
  }
  ...
  async getDialogues(user) {
    ...
    const memoryPath = path6.join(this.config.dataDir, "dialogues", `${userId}.txt`);
    if (!fs6.existsSync(memoryPath))
      return [];
    let memoryContent = [];
    try {
      memoryContent = JSON.parse(fs6.readFileSync(memoryPath, "utf-8"));
    } catch {
      return [];
    }
    ...
    try {
      fs6.writeFileSync(memoryPath, JSON.stringify(replacedContent, null, 2));
    } catch {
    }
...
·不使用markdown等标记语言，直接书写即可`
  ...
  async generatePortrait(session, user, apiClient) {
    ...
    logger6.info(`用户 ${user.userid} 画像生成中...`);
    try {
...
var logger7 = new import_koishi8.Logger("satori-ai-mood");
...
var abstractGame = class {
  ...
  async endGame(session, ctx) {
    ...
    logger8.info(`时间：${session.timestamp}，用户${session.userId}试图结束用户${game.getPlayerID()}开启的游戏，上次互动时间${game.getLastActionTime()}`);
...
// src/gamefencing.ts
var logger9 = new import_koishi10.Logger("satori-game-fencing");
...
var logger10 = new import_koishi11.Logger("satori-game-onetouch");
var SKILL_MAP = {
  // 基础技能
  "-1": {},
...
当前总分数：${this.bonus}`;
  ...
  aiSearchEntrance() {
    ...
    logger10.info(`AI选择的动作：${bestMove[0]}碰${bestMove[1]}, 分数：${bestScore}`);
    if (this.turnCount > 5)
...
// src/game.ts
var logger11 = new import_koishi12.Logger("satori-game");
var Game = class {
  static {
    __name(this, "Game");
  }
  ...
  async startGame(session, gameName, args) {
    ...
    logger11.info(`游戏${gameName}已开始于${session.channelId}`);
  }
  // 结束当前游戏
  async endGame(session) {
    ...
    if (await this.availableGames.get(gameName).endGame(session, this.context)) {
      this.channelGames.delete(session.channelId);
      logger11.info(`${session.channelId}的游戏已结束`);
    }
  }
...
var logger12 = new import_koishi13.Logger("satori-ai-broadcast");
var BroadcastManager = class {
  ...
  async createBroadcast(message, img) {
    ...
    logger12.info(`广播已创建 ID: ${id}`);
    return `广播已创建 ID: ${id}`;
  }
  async deleteBroadcast(id) {
    if (this.broadcastMessages.has(id)) {
      this.broadcastMessages.delete(id);
      logger12.info(`广播已删除 ID: ${id}`);
      return "已删除广播：" + id;
    } else {
      logger12.warn(`未找到广播 ID: ${id}`);
      return "未找到该广播";
    }
  }
...
var broadcastMessage = class {
  ...
  async sendMessage(session) {
    ...
    try {
      ...
      logger12.info(`向群 ${channelId} 广播: ${this.message}`);
    } catch (error) {
      logger12.error(`广播失败 ${channelId}: ${error}`);
    }
  }
...
var logger13 = new import_koishi14.Logger("satori-weather");
var WeatherManager = class {
  ...
  async generateJWT() {
    ...
    try {
      ...
      logger13.error("生成 JWT 失败: " + (error.message || error.status || error));
      return null;
    }
  }
    ...
    try {
      ...
      if (buf.length >= 2 && buf[0] === 31 && buf[1] === 139) {
        try {
          ...
          logger13.warn("Gzip 解压失败，尝试按文本解析: " + (e.message || e));
        }
      }
      ...
      logger13.error("fetchJson 请求失败: " + (error.message || error.status || error));
      throw error;
    }
  }
  // 更新用户位置信息
  async updateLocation(session, location) {
    ...
    try {
      ...
      if (Array.isArray(results) && results.length > 0) {
        ...
        logger13.info(`用户 ${session.userId} 的位置已更新为 ${fullLocation}`);
        ...
        logger13.warn(`未找到用户 ${session.userId} 的位置，输入为 "${location}"`);
        return "未能找到该位置，请确认输入是否正确。";
      }
    } catch (error) {
      logger13.error(`未能更新用户 ${session.userId} 的位置: ${error.message || error.status}`);
  ...
  async getWeatherInfo(session) {
    ...
    try {
      ...
      if (weatherData) {
        ...
        logger13.info(`为用户 ${session.userId} 获取了最新的天气信息`);
        return weatherInfo;
      } else {
        logger13.warn(`未能获取用户 ${session.userId} 的天气信息，位置ID为 "${location}"`);
        return "未能获取天气信息，请稍后再试。";
      }
    } catch (error) {
      logger13.error(`未能获取用户 ${session.userId} 的天气信息: ${error.message || error.status}`);
...
var Galgame = class {
  ...
  currentId = {};
  loadEvent(eventName) {
    const filePath = path7.resolve(this.config.dataDir, "event", `${eventName}.json`);
  ...
  async sendStoryContent(session, content) {
    ...
    if (content.imageName) {
      const imagePath = path7.resolve(this.config.dataDir, "event", content.imageName);
      if (fs7.existsSync(imagePath)) {
        ...
        await session.send(import_koishi15.h.image(buffer, mime));
      }
    }
  }
  // 根据文本估算自动推进的延迟（毫秒）
  ...
  async handleEventProgress(session, event) {
    ...
    if (content.isEnding) {
      ...
      try {
        const portraitPath = path7.join(this.config.dataDir, "UserPortrait", `${userId}.txt`);
        if (fs7.existsSync(portraitPath)) {
          const appendText = `
近期你和他进行了约会：${event.eventName}`;
          fs7.appendFileSync(portraitPath, appendText, "utf-8");
        }
      } catch (e) {
      }
      return;
...
${rewardParts.join("\n")}` : "";
    ...
    if (ending.image) {
      const imagePath = path7.resolve(this.config.dataDir, "event", ending.image);
      if (fs7.existsSync(imagePath)) {
        const buffer = fs7.readFileSync(imagePath);
        const ext = path7.extname(imagePath).toLowerCase();
        const mime = ext === ".png" ? "image/png" : ext === ".webp" ? "image/webp" : "image/jpeg";
  ...
  async startEvent(session, eventName) {
    ...
    if (!eventName) {
      const eventDir = path7.join(this.config.dataDir, "event");
      if (!fs7.existsSync(eventDir)) {
        await this.initialize();
      }
      const events = fs7.readdirSync(eventDir).filter((file) => file.endsWith(".json")).map((file) => this.loadEvent(file.replace(".json", ""))).filter((event2) => event2 && user.favorability >= event2.favorabilityThreshold).map((event2) => event2.eventName);
  ...
  async initialize() {
    const eventDir = path7.join(this.config.dataDir, "event");
    if (!fs7.existsSync(eventDir)) {
      fs7.mkdirSync(eventDir, { recursive: true });
    }
    const files = fs7.readdirSync(eventDir);
    if (files.length === 0) {
      const exampleEvent = {
        eventName: "示例事件",
...
支持字段：
 ...
 此段即演示isNarration和imageName效果`,
      ...
      const exampleFilePath = path7.resolve(eventDir, "示例事件.json");
      fs7.writeFileSync(exampleFilePath, JSON.stringify(exampleEvent, null, 2), "utf-8");
      const exampleImagePath = path7.resolve(eventDir, "example.png");
      try {
        if (!fs7.existsSync(exampleImagePath)) {
          const imageBuffer = Buffer.from(exampleimg, "base64");
          fs7.writeFileSync(exampleImagePath, imageBuffer);
        }
      } catch (e) {
      }
    }
...
// src/index.tsx
var logger14 = new import_koishi16.Logger("satori-ai");
...
var SAT = class extends Sat {
  ...
  async waitForPuppeteer(ctx, retries = 20, intervalMs = 1e3) {
    for (let i = 0; i < retries; i++) {
      if (ctx.puppeteer) {
        ...
        refreshPuppeteer(ctx);
        logger14.info(`Puppeteer 在尝试 ${i + 1}/${retries} 中就绪`);
        return true;
      }
      await new Promise((resolve4) => setTimeout(resolve4, intervalMs));
    }
    logger14.warn("等待 Puppeteer 超时，未能在指定重试次数内初始化");
    return false;
  }
  ...
  registerCommands(ctx) {
    ctx.command("sat [...args]", { authority: this.config.authority }).alias(...this.config.alias).action(async ({ session }, ...prompts) => this.handleSatCommand(session, prompts.join(" ")));
  ...
  async handleSatCommand(session, prompt) {
    ...
    if (this.emojiManager.shouldSendEmoji()) {
      try {
        ...
        if (emojiPath) {
          const buffer = await import_fs.promises.readFile(emojiPath);
          const ext = path8.extname(emojiPath).slice(1) || "gif";
          await session.send(import_koishi16.h.image(buffer, "image/" + ext));
        }
        logger14.info(`发送表情包: ${emojiPath} 给用户: ${session.username} (${session.userId})`);
      } catch (e) {
        logger14.error("Failed to send emoji:", e);
      }
    }
  ...
  async handleAuxiliaryDialogue(session, prompt, response) {
    ...
    if (this.config.enable_auxiliary_LLM && !response.error && response.content) {
      ...
      if (result.error) {
        logger14.error(`辅助判断失败：${result.content}`);
      } else {
        logger14.info(`辅助判断：${result.content}`);
  ...
  async processInput(session, prompt) {
    ...
    if (this.config.log_ask_response)
      logger14.info(`用户 ${session.username}：${processedPrompt}`);
    return censored;
  }
  // 生成回复
  async generateResponse(session, prompt) {
    if (this.getChannelParallelCount(session) > this.config.max_parallel_count) {
      logger14.info(`频道 ${session.channelId} 并发数过高(${this.getChannelParallelCount(session)})，${session.username}等待中...`);
    }
    let waitTimes = 0;
    while (this.getChannelParallelCount(session) >= this.config.max_parallel_count) {
      waitTimes++;
      if (waitTimes > 60) {
        logger14.error(`并发等待超时：channel=${session.channelId}`);
    ...
    try {
      logger14.info(`频道 ${session.channelId} 处理：${session.userId},剩余${this.getChannelParallelCount(session)}并发`);
  ...
  async getChatResponse(user, messages, prompt) {
    ...
    logger14.info(`API 调用耗时: ${Date.now() - startTime}ms`);
    if (this.config.log_ask_response) {
      if (this.config.enable_favorability && this.config.enable_mood)
        logger14.info(`Satori AI：（心情值：${this.moodManager.getMoodValue(user.userid)}）${response.content}`);
      else
        logger14.info(`Satori AI：${response.content}`);
    }
    if (!response.error) {
      ...
      if (filterTime > 1e3) {
        logger14.warn(`filterResponse 耗时异常: ${filterTime}ms, 内容长度: ${response.content?.length || 0}`);
      }
    }
    return response;
  }
...
`;
  ...
  async formatResponse(session, response, auxiliaryResult, reasoningContent) {
    ...
    if (eye && reasoningContent) {
      try {
        ...
        logger14.warn("解析 reasoningContent 时出错：" + e.message);
      }
    }
    if (catEar)
      response += " 喵~";
    ...
    if (this.ctx.puppeteer && !this.puppeteerReady()) {
      logger14.info("Puppeteer 未就绪，正在重新初始化 Puppeteer...");
    ...
    if (firstReasoning.length > 15) {
      ...
      try {
        ...
        if (renderTime > 5e3) {
          logger14.warn(`wrapInHTML 渲染耗时异常: ${renderTime}ms, 内容长度: ${firstReasoning.length}`);
        }
      } catch (error) {
        logger14.error(`wrapInHTML 渲染失败: ${error}`);
      }
    }
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response, this.config.min_sentences_length, this.config.max_sentences_length).map((text) => import_koishi16.h.text(text));
      for (const sentence of sentences) {
        await session.send(sentence);
        await new Promise((resolve4) => setTimeout(resolve4, this.config.time_interval));
      }
      return null;
    }
    return response;
  ...
  addUserToWarnList(session, auxiliaryResult) {
    ...
    if (auxiliaryResult.includes("好感↓")) {
      logger14.info(`在群${session.channelId}中，${session.username}骚扰我！`);
...
`;
  ...
  async addCommonSense(session, content) {
    ...
    const filePath = path8.join(this.config.dataDir, "common_sense.txt");
  ...
  async addGroupCommonSense(session, content, groupId) {
    ...
    const filePath = path8.join(this.config.dataDir, `group_sense`, `${id}.txt`);
    await this.memoryManager.saveLongTermMemory(session, [{
      role: "user",
      content: content.replace(/\"/g, "'").trim()
    }], filePath);
  ...
  async dispose() {
    if (this.puppeteer) {
      try {
        ...
        logger14.info("Puppeteer 已成功释放");
      } catch (error) {
        logger14.warn("释放 Puppeteer 时出错", error);
      } finally {
        this.puppeteer = null;
      }
    }
