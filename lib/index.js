...
发送"结束游戏"退出游戏`);
  ...
  async processInput(input) {
    if (input === "游戏规则")
      return await wrapInHTML2(instuction);
    if (this.turnCount === 0)
  ...
  async buildTurnResult(result, aiResult) {
    const createStatusBar = /* @__PURE__ */ __name((value, max, width) => `[${"■".repeat(Math.round(value / max * width))}${"□".repeat(width - Math.round(value / max * width))}]`, "createStatusBar");
...
我的行动：${aiResult}
  ...
  applyEffectToEnemy(self, target, effect, isCombo) {
    if (effect.pierceDamage)
      target.hp -= effect.pierceDamage + (self.strength || 0);
  ...
  buildResultMessage(effect, isCombo, name, selfStatu, enemyStatu) {
    let msg = [];
    if (name)
      msg.push(`${name}!`);
    if (effect.pierceDamage)
      msg.push(`对对方造成穿刺伤害${effect.pierceDamage + (selfStatu.strength || 0)}`);
    if (effect.damage)
      msg.push(`对对方造成${effect.damage + (selfStatu.strength || 0)}伤害`);
    ...
    if (enemyStatu.status !== 2 /* lastStunned */) {
      if (effect.stun)
        msg.push(`眩晕对方`);
      if (effect.weakStun && enemyStatu.shield === 0)
        msg.push(`弱眩晕对方`);
    }
    if (effect.selfStun && selfStatu.status !== 2 /* lastStunned */)
      msg.push(`眩晕自己`);
    if (selfStatu.bleed > 0)
      msg.push(`自身受到流血伤害${selfStatu.bleed}`);
    if (isCombo)
      msg.unshift("触发组合技！\n");
    return msg.join(" ");
  }
