...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    ...
    import_koishi5.Schema.object({
      enable_mood: import_koishi5.Schema.boolean().default(false).description("是否开启心情系统（通过屏蔽词降低心情，正常聊天心情+1，每天重置为0）"),
      max_mood: import_koishi5.Schema.number().default(10).description("心情上限"),
      value_of_input_mood: import_koishi5.Schema.number().default(15).description("输入触发屏蔽词每次扣除的心情"),
      output_censor_mood: import_koishi5.Schema.boolean().default(false).description("通过输出屏蔽词扣除心情,在dataDir中的output_censor.txt修改)"),
      value_of_output_mood: import_koishi5.Schema.number().default(10).description("输出触发屏蔽词每次扣除的心情"),
      mood_div_1: import_koishi5.Schema.number().default(-5).description("心情正常-烦躁分界线"),
      mood_prompt_0: import_koishi5.Schema.string().role("textarea").default("你现在的心情是：有点烦躁").description("烦躁心情补充设定"),
      mood_div_2: import_koishi5.Schema.number().default(-30).description("烦躁-生气分界线"),
      ...
      min_pocket_money: import_koishi5.Schema.number().default(200).description("随机p点最小值"),
      max_pocket_money: import_koishi5.Schema.number().default(400).description("随机p点最大值"),
      pocket_money_cost: import_koishi5.Schema.number().default(5).description("消耗心情换p点的心情值")
...
// src/index.tsx
var logger7 = new import_koishi8.Logger("satori-ai");
var SAT = class extends Sat {
  ...
  async handleAuxiliaryDialogue(session, prompt, response) {
    ...
    if (this.config.enable_auxiliary_LLM && !response.error && response.content) {
      ...
      if (result.error) {
        logger7.error(`辅助判断失败：${result.content}`);
      } else {
        logger7.info(`辅助判断：${result.content}`);
  ...
  async processInput(session, prompt) {
    ...
    if (this.config.log_ask_response)
      logger7.info(`用户 ${session.username}：${processedPrompt}`);
    return censored;
  }
  // 生成回复
  async generateResponse(session, prompt) {
    if (this.getChannelParallelCount(session) > this.config.max_parallel_count) {
      logger7.info(`频道 ${session.channelId} 并发数过高(${this.getChannelParallelCount(session)})，${session.username}等待中...`);
    }
    ...
    logger7.info(`频道 ${session.channelId} 处理：${session.userId},剩余${this.getChannelParallelCount(session)}并发`);
    const user = await getUser(this.ctx, session.userId);
    let response = await this.apiClient.chat(user, await messages);
    if (this.config.log_ask_response) {
      if (this.config.enable_favorability && this.config.enable_mood)
        logger7.info(`Satori AI：（心情值：${this.moodManager.getMoodValue(user.userid)}）${response.content}`);
      else
        logger7.info(`Satori AI：${response.content}`);
    }
...
`;
  ...
  async formatResponse(session, response, auxiliaryResult) {
    ...
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response).map((text) => import_koishi8.h.text(text));
