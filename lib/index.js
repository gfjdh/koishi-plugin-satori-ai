...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    ...
    import_koishi5.Schema.object({
      ...
      cd_for_OneTouch: import_koishi5.Schema.number().default(3600).description("每人一碰一游戏cd时间（秒）"),
...
你输了！发送结束游戏退出`;
  ...
  buildTurnResult(result, aiResult) {
    return `你的行动：${result}
我的行动：${aiResult}
  ...
  applyEffectToSelf(self, effect) {
    if (effect.heal)
      self.hp += effect.heal;
    if (effect.shield)
      self.shield = Math.min(self.shield + effect.shield, 3);
  ...
  buildResultMessage(effect, isCombo, name) {
    let msg = [];
    if (name)
      msg.push(`${name}!`);
    if (effect.pierceDamage)
      msg.push(`对对方造成穿刺伤害${effect.pierceDamage}`);
    if (effect.damage)
      msg.push(`对对方造成${effect.damage}伤害`);
