...
var require_zh = __commonJS({
  "src/locales/zh.yml"(exports, module2) {
    module2.exports = { commands: { sat: { description: "AI聊天", messages: { notExists: "请先签到哦......", censor: "不合规，请检查输入内容是否含有违禁词。", "no-prompt": "你好。使用说明可以看我的空间哦。", tooManyEnglishLetters: "请不要用这么多英文......", err: "寄！{0}", tooLong: "你的话太多了。", content_tooLong: "这个问题有点复杂，你还是去问别人吧。", "duplicate-dialogue": "这个刚刚说过了吧......", clean: "舒服了", block1: "我讨厌你！", block2: "我讨厌你们！" } }, "sat.clear": { description: "清空所有人的会话" }, "sat.common_sense": { description: "添加常识", messages: { "no-prompt": "你想让我知道什么？", succeed: "我知道了，{0}" } } } };
  }
});
...
((Sat2) => {
  Sat2.Config = import_koishi.Schema.intersect([
    ...
    import_koishi.Schema.object({
      ...
      enable_self_memory: import_koishi.Schema.boolean().default(true).description("是否启用自发言记忆（启用容易发生复读）"),
      ...
      temperature: import_koishi.Schema.number().role("slider").min(0).max(2).step(0.01).default(0.5).description("温度"),
      authority: import_koishi.Schema.number().role("slider").min(0).max(5).step(1).description("允许使用的最低权限").default(1),
      enable_fixed_dialogues: import_koishi.Schema.boolean().default(true).description("是否启用固定对话（在dataDir中的fixed_dialogues.json修改）"),
      ...
      random_min_tokens: import_koishi.Schema.number().default(20).description("随机触发的最小长度"),
      randnum: import_koishi.Schema.number().role("slider").min(0).max(1).step(0.01).default(0).description("在群聊中随机触发对话的概率，如需关闭可设置为 0"),
    ...
    import_koishi.Schema.object({
      enable_favorability: import_koishi.Schema.boolean().default(false).description("是否开启好感度系统（建议添加p-qiandao插件）"),
...
var SAt = class extends Sat {
  ...
  constructor(ctx, config) {
    ...
    ctx.command("sat.clear", "清空会话", { authority: 1 }).action(({ session }) => {
      return this.clear(session);
    });
    ctx.command("sat.common_sense <text:text>", "添加常识", { authority: 3 }).alias("添加常识").action(async ({ session }, ...prompt) => {
  ...
  async sat(session, prompt) {
    let notExists, user;
    if (this.pluginConfig.enable_favorability) {
      notExists = await isTargetIdExists(this.ctx, session.userId);
      if (notExists)
        await this.ctx.database.create("p_system", { userid: session.userId, usersname: session.username, p: 0, favorability: 0 });
      user = await this.ctx.database.get("p_system", { userid: session.userId });
      if (user[0].favorability < -30 && user[0].favorability > -900)
        return session.text("commands.sat.messages.block1");
      const englishLettersCount = (prompt.match(/[a-zA-Z]/g) || []).length;
      if (user[0].favorability < 50 && englishLettersCount > 8)
    ...
    if (this.pluginConfig.enableContext) {
      ...
      if (duplicateDialogue) {
        if (this.pluginConfig.enable_favorability) {
          const user2 = await this.ctx.database.get("p_system", { userid: session.userId });
          const newFavorability = user2[0].favorability - 1;
          await this.ctx.database.set("p_system", { userid: user2[0].userid }, { favorability: newFavorability });
        }
        return session.text("commands.sat.messages.duplicate-dialogue");
      }
    }
    const fs2 = require("fs");
    if (this.pluginConfig.enable_fixed_dialogues) {
      const fixedDialoguesPath = path.join(this.pluginConfig.dataDir, "fixed_dialogues.json");
      if (!fs2.existsSync(fixedDialoguesPath)) {
        ...
        fs2.writeFileSync(fixedDialoguesPath, JSON.stringify(defaultDialogues, null, 2), "utf-8");
      }
      const fixedDialogues = JSON.parse(fs2.readFileSync(fixedDialoguesPath, "utf-8"));
      ...
      const matchedDialogues = fixedDialogues.filter((dialogue) => {
        ...
        const favorabilityMatch = dialogue.favorabilityRange ? user[0].favorability >= dialogue.favorabilityRange[0] && user[0].favorability <= dialogue.favorabilityRange[1] : true;
    ...
    if (this.pluginConfig.enableContext) {
      ...
      if (fs2.existsSync(filePath)) {
        ...
        if (sortedMatches.length > 0) {
          this.personality["人格"][0].content += appendTopMatches(JSON.parse(fs2.readFileSync(filePath, "utf-8")), sortedMatches, 10, "这是你和发言者较久之前的对话内容：");
        }
      }
      const commonSenseFilePath = path.join(this.pluginConfig.dataDir, "common_sense.txt");
      let commonSenseContent;
      if (fs2.existsSync(commonSenseFilePath))
      ...
      const Date2 = await getCurrentDate();
      this.personality["人格"][0].content += `
当前日期: ${Date2}
`;
...
`;
  ...
  async chat(msg, sessionid, session) {
    ...
    if (this.pluginConfig.enable_self_memory)
