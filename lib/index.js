...
var APIClient = class {
  ...
  async chat(user, messages) {
    if (user.userid == "Alice")
      return { content: "<p>(系统)这是一个测试句子。这个句子稍长一些，包含多个标点符号！这是一个特别长的句子，需要超过最大长度限制的句子应该被保留原样，但这种情况在实际使用中应该尽量避免。最后？这是一个需要合并的短句！;</p>", error: false };
...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    ...
    import_koishi5.Schema.object({
      ...
      enable_gobang: import_koishi5.Schema.boolean().default(false).description("是否开启五子棋游戏（需要puppeteer支持）"),
      channel_id_for_gobang: import_koishi5.Schema.array(String).description("启用五子棋的游戏频道id（不建议太多，第一个会被公示）"),
      cd_for_gobang: import_koishi5.Schema.number().default(3600).description("每人五子棋游戏cd时间（秒）"),
...
var goBangSingleGame = class extends abstractGameSingleGame {
  ...
  startGame = () => {
    ...
    const getCornerCoordinate = /* @__PURE__ */ __name((i) => {
  ...
  endGame = async () => {
    ...
    if (this.winningFlag === 1 /* win */ && this.playerFlag === 1) {
      bonusLevel -= 1;
    } else if (this.winningFlag === 2 /* lose */ && this.playerFlag === 1) {
      bonusLevel += 1;
    }
...
var SKILL_MAP = {
  ...
  // 组合技
  "1+8": { damage: 14, bleed: 3, selfStun: true, name: "空尖弹" },
  ...
  "1+9": { pierceDamage: 5, bleed: 5, strengthChange: -1, name: "收割" },
  "4+4": { shield: 4, heal: 5, name: "壁垒" },
  "8+8": { damage: 12, damageTimes: 2, selfStun: true, name: "双持" },
  "1+2": { pierceDamage: 3, bleed: 3, stun: true, name: "弱点刺击" },
  "3+4": { name: "防御反击", shield: 1, counterAttack: 15 },
  "7+7": { pierceDamage: 7, destroyShield: 5, name: "穿龙枪" },
  "7+8": { pierceDamage: 12, destroyShield: 3, selfStun: true, name: "穿甲弹" },
  "2+6": { pierceDamage: 5, heal: 6, selfbleed: -1, selfstrengthChange: 1, stun: true, name: "点辰" },
...
发送"结束游戏"退出游戏`);
  ...
  async buildTurnResult(result, aiResult) {
    ...
    return wrapInHTML2(`你的行动：${result}
...
我的行动：${aiResult}
  ...
  applyEffectToEnemy(self, target, effect, isCombo) {
    if (effect.pierceDamage)
      target.hp -= Math.max(effect.pierceDamage + (self.strength || 0), 0);
    if (effect.destroyShield)
      target.shield = Math.max(0, target.shield - effect.destroyShield);
    if (effect.bleed)
      target.bleed += effect.bleed;
    if (effect.strengthChange)
      target.strength = target.strength + effect.strengthChange;
    if (effect.addLeft)
      target.left = (target.left + effect.addLeft) % 10;
    if (effect.addRight)
      target.right = (target.right + effect.addRight) % 10;
    if (effect.damage) {
      ...
      for (let i = 0; i < effect.damageTimes; i++) {
        ...
        if (!blocked) {
          ...
          if (effect.weakStun && target.status !== 2 /* lastStunned */) {
            target.status = 1 /* Stunned */;
          }
        }
      }
    }
    ...
    if (effect.stun && target.status !== 2 /* lastStunned */)
      target.status = 1 /* Stunned */;
    return target;
  }
  // heal?: number // 治疗
  ...
  applyEffectToSelf(enemy, self, effect) {
    if (effect.heal)
      self.hp += effect.heal;
    if (effect.shield)
    ...
    if (effect.selfbleed)
      self.bleed = self.bleed + effect.selfbleed;
    if (self.bleed > 0)
      self.hp -= self.bleed;
  ...
  buildResultMessage(effect, isCombo, self, enemy) {
    ...
    if (effect.pierceDamage)
      msg.push(`对对方造成穿刺伤害${effect.pierceDamage + (self.strength || 0)}`);
    if (effect.damage)
      msg.push(`对对方造成${effect.damage + (self.strength || 0)}伤害`);
  ...
  generateChat(Score) {
    if (this.lastScore < 1e5 && Score > 1e5) {
      return "我觉得你要输了哦~";
    }
  }
  instuction = `游戏说明：
  ...
  反击：下一回合若对方行动中有普通攻击，则对方受到反击层数点穿刺伤害，此效果仅持续一回合
  ...
  async resultJudge(result, ctx, session) {
    ...
    switch (result.gameName) {
      ...
      case "一碰一":
        ...
        } else if (oneTouchResult.win === 3 /* draw */)
          session.send("平局，稍后再战吧");
        else {
          updateUserP(ctx, user, -oneTouchBonus);
