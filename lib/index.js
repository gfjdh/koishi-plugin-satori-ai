...
var SAt = class extends Sat {
  ...
  async sat(session, prompt) {
    ...
    if (this.pluginConfig.enableContext) {
      ...
      const charactersToRemove = ["的", "一", "是", "了", "我", "不", "人", "在", "他", "有", "这", "个", "上", "们", "来", "到", "时", "大", "地", "为", "子", "中", "你", "说", "生", "国", "年", "着", "就", "那", "和", "要", "她", "出", "也", "得", "里", "后", "自", "以", "会"];
      ...
      if (fs.existsSync(filePath)) {
        ...
        const dialogues2 = JSON.parse(fileContent).filter((msg) => msg.role === "user");
        let matchCounts = [];
        for (let i = 0; i < dialogues2.length; i++) {
          const dialogue = dialogues2[i];
          let count = 0;
...
我的名字: ${session.username}`;
      ...
      if (debug)
        logger.info(this.personality["人格"][0].content.slice(-800));
