...
var MemoryManager = class {
  ...
  async saveLongTermMemory(session, dialogues, filePath = "") {
    ...
    await fs2.promises.writeFile(filePath, JSON.stringify(updated, null, 2), "utf-8");
  }
  // 记忆检索
  async searchMemories(session, type = "user") {
    ...
    if (type === "user") {
      ...
      await fs2.promises.writeFile(filePath, JSON.stringify(updatedEntries, null, 2), "utf-8");
    }
...
`;
  ...
  async ensureMemoryFile(filePath) {
    ...
    if (!fs2.existsSync(filePath)) {
      await fs2.promises.writeFile(filePath, "[]", "utf-8");
    }
  }
  // 加载记忆文件
...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    import_koishi5.Schema.object({
      ...
      enable_reasoner_like: import_koishi5.Schema.boolean().default(true).description("是否启用非深度思考模型模仿思维链（可以在节省成本的同时提高效果，但是模型经常不按规定格式输出，所以不推荐开启）"),
    ...
    import_koishi5.Schema.object({
      ...
      max_output_tokens: import_koishi5.Schema.number().description("最大返回长度(不含思维链)（token数）").default(4096),
...
var SAT = class extends Sat {
  ...
  async generateResponse(session, prompt) {
    ...
    while (this.getChannelParallelCount(session) >= this.config.max_parallel_count) {
      waitTimes++;
      if (waitTimes > 60) {
        logger13.error(`并发等待超时：channel=${session.channelId}`);
  ...
  async getChatResponse(user, messages, prompt) {
    ...
    logger13.info(`API 调用耗时: ${Date.now() - startTime}ms`);
    if (this.config.log_ask_response) {
    ...
    if (!response.error) {
      ...
      if (filterTime > 1e3) {
        logger13.warn(`filterResponse 耗时异常: ${filterTime}ms, 内容长度: ${response.content?.length || 0}`);
      }
    }
    return response;
  }
  ...
  async formatResponse(session, response, auxiliaryResult, reasoningContent) {
    ...
    if (firstReasoning.length > 15) {
      ...
      try {
        ...
        if (renderTime > 5e3) {
          logger13.warn(`wrapInHTML 渲染耗时异常: ${renderTime}ms, 内容长度: ${firstReasoning.length}`);
        }
      } catch (error) {
        logger13.error(`wrapInHTML 渲染失败: ${error}`);
      }
    }
  ...
  async dispose() {
    if (this.puppeteer) {
      try {
        ...
        logger13.info("Puppeteer 已成功释放");
      } catch (error) {
        logger13.warn("释放 Puppeteer 时出错", error);
      } finally {
        this.puppeteer = null;
      }
    }
