    ...
    for (let i = 1; i <= this.config.maxRetryTimes; i++) {
      try {
        ...
        if (this.config.reasoning_content)
          logger.info(`思维链: ${reasoning_content || "无"}`);
...
var logger3 = new import_koishi3.Logger("satori-ai");
...
async function handlePrivateMessage(SAT2, session) {
  ...
  if (content)
    return await SAT2.handleMiddleware(session, content);
}
...
async function handleMentionMessage(SAT2, session) {
  ...
  if (message)
    return await SAT2.handleMiddleware(session, message);
}
...
async function handleRandomTrigger(SAT2, session, config) {
  ...
  return await SAT2.handleMiddleware(session, session.content);
}
__name(handleRandomTrigger, "handleRandomTrigger");

// src/types.ts
var import_koishi4 = require("koishi");
var Sat = class extends import_koishi4.Service {
  static {
    __name(this, "Sat");
  }
  static inject = {
...
((Sat2) => {
  Sat2.Config = import_koishi4.Schema.intersect([
    import_koishi4.Schema.object({
      baseURL: import_koishi4.Schema.string().default("https://api.deepseek.com").description("请求地址"),
      key: import_koishi4.Schema.union([
        import_koishi4.Schema.array(String).role("secret"),
        import_koishi4.Schema.transform(String, (value) => [value])
      ]).default([]).role("secret").description("api_key"),
      appointModel: import_koishi4.Schema.string().default("deepseek-reasoner").description("主模型"),
      auxiliary_LLM_URL: import_koishi4.Schema.string().default("https://api.deepseek.com").description("辅助模型请求地址"),
      auxiliary_LLM: import_koishi4.Schema.string().default("deepseek-chat").description("辅助模型(用于好感度调整等功能，如不需要可不填，建议使用低成本模型"),
      auxiliary_LLM_key: import_koishi4.Schema.union([
        import_koishi4.Schema.array(String).role("secret"),
        import_koishi4.Schema.transform(String, (value) => [value])
      ]).default([]).role("secret").description("辅助模型api_key"),
      prompt: import_koishi4.Schema.string().role("textarea").description("人格设定")
    }).description("基础设置"),
    import_koishi4.Schema.object({
      alias: import_koishi4.Schema.array(String).default(["ai"]).description("触发命令;别名"),
      authority: import_koishi4.Schema.number().role("slider").min(0).max(5).step(1).description("允许使用的最低权限").default(1),
      max_tokens: import_koishi4.Schema.number().description("最大请求长度（字符数）").default(100),
      content_max_tokens: import_koishi4.Schema.number().description("最大回答长度（思维链+输出token）").default(4096),
      content_max_length: import_koishi4.Schema.number().description("最大回答长度（仅输出，字符数）").default(100),
      message_max_length: import_koishi4.Schema.number().description("最大频道上下文长度（条数）").default(10),
      temperature: import_koishi4.Schema.number().role("slider").min(0).max(2).step(0.01).default(0.5).description("温度"),
      frequency_penalty: import_koishi4.Schema.number().default(0).description("频率惩罚"),
      presence_penalty: import_koishi4.Schema.number().default(0).description("存在惩罚"),
      maxRetryTimes: import_koishi4.Schema.number().default(10).description("报错后最大重试次数"),
      retry_delay_time: import_koishi4.Schema.number().default(5e3).description("每次重试延迟时间"),
      max_parallel_count: import_koishi4.Schema.number().default(2).description("频道最大并行请求数"),
      log_system_prompt: import_koishi4.Schema.boolean().default(false).description("是否在日志中输出系统提示"),
      log_reasoning_content: import_koishi4.Schema.boolean().default(true).description("是否在日志中输出思维链"),
      log_ask_response: import_koishi4.Schema.boolean().default(true).description("是否在日志中输出问答")
    }).description("请求设置"),
    import_koishi4.Schema.object({
      enable_self_memory: import_koishi4.Schema.boolean().default(true).description("是否启用模型自发言记忆（仅短期）"),
      personal_memory: import_koishi4.Schema.boolean().default(true).description("是否启用按人记忆（否则将群内所有人视为同一个用户）"),
      channel_dialogues: import_koishi4.Schema.boolean().default(true).description("是否获取群聊内最近对话（包括不对bot说的）"),
      channel_dialogues_max_length: import_koishi4.Schema.number().default(20).description("群聊内最近对话最大长度(条数)"),
      bracket_filter: import_koishi4.Schema.boolean().default(false).description("是否启用括号过滤，开启后在写入短期记忆时会过滤掉括号内的内容，用于缓解复读问题"),
      memory_filter: import_koishi4.Schema.string().role("textarea").default("示例1-示例2").description("短期记忆过滤词，使用“-”分隔，含有过滤词的那一句不会被记忆，用于缓解复读问题"),
      dataDir: import_koishi4.Schema.string().default("./data/satori_ai").description("聊天记录保存位置（长期记忆）"),
      memory_block_words: import_koishi4.Schema.array(String).default(["好感"]).description("记忆屏蔽词"),
      remember_min_length: import_koishi4.Schema.number().description("触发保存到记忆的长度").default(20),
      common_topN: import_koishi4.Schema.number().default(5).description("常识记忆检索最大匹配数"),
      dailogues_topN: import_koishi4.Schema.number().default(5).description("对话记忆检索最大匹配数"),
      enable_fixed_dialogues: import_koishi4.Schema.boolean().default(false).description("是否启用固定对话（在dataDir中的fixed_dialogues.json修改）")
    }).description("记忆设置"),
    import_koishi4.Schema.object({
      max_usage: import_koishi4.Schema.tuple([Number, Number, Number, Number, Number]).default([40, 240, 3e3, 9999, 0]).description("每日最大使用次数(对应用户level0~level4)(0为不限制)"),
      private: import_koishi4.Schema.boolean().default(false).description("开启后私聊AI可触发对话, 不需要使用指令"),
      mention: import_koishi4.Schema.boolean().default(false).description("开启后机器人被提及(at/引用)可触发对话"),
      nick_name: import_koishi4.Schema.boolean().default(true).description("是否使用昵称触发对话（发言中含有昵称时）"),
      nick_name_list: import_koishi4.Schema.array(String).default(["ai"]).description("昵称列表"),
      duplicateDialogueCheck: import_koishi4.Schema.boolean().default(true).description("是否检查重复对话"),
      enable_online_user_check: import_koishi4.Schema.boolean().default(true).description("在未回答而再次提问时是否提示用户有未完成的对话"),
      random_min_tokens: import_koishi4.Schema.number().default(20).description("随机触发对话的最小长度"),
      randnum: import_koishi4.Schema.number().role("slider").min(0).max(1).step(0.01).default(0).description("在群聊中随机触发对话的概率，如需关闭可设置为 0"),
      sentences_divide: import_koishi4.Schema.boolean().default(true).description("是否分句发送"),
      time_interval: import_koishi4.Schema.number().default(1e3).description("每句话的时间间隔"),
      reply_pointing: import_koishi4.Schema.boolean().default(true).description("是否在与多人同时对话时显示回复指向")
    }).description("对话设置"),
    import_koishi4.Schema.object({
      enable_favorability: import_koishi4.Schema.boolean().default(false).description("是否开启好感度系统(每次对话默认+1好感度)"),
      max_favorability_perday: import_koishi4.Schema.number().default(100).description("每日有效(引发好感度变化)对话次数上限"),
      input_censor_favorability: import_koishi4.Schema.boolean().default(false).description("是否开启好感度审查(通过输入屏蔽词扣除好感)"),
      value_of_input_favorability: import_koishi4.Schema.number().default(15).description("输入触发屏蔽词每次扣除的好感度"),
      output_censor_favorability: import_koishi4.Schema.boolean().default(false).description("通过输出屏蔽词扣除好感,在dataDir中的output_censor.txt修改)"),
      value_of_output_favorability: import_koishi4.Schema.number().default(15).description("输出触发屏蔽词每次扣除的好感度"),
      enable_auxiliary_LLM: import_koishi4.Schema.boolean().default(false).description("是否使用辅助大模型判断好感度增减(量与输入屏蔽词每次扣除的好感度相关,不稳定，慎用)"),
      offset_of_fafavorability: import_koishi4.Schema.number().default(3.5).description("辅助大模型好感度偏移量(越大越容易扣好感度)"),
      visible_favorability: import_koishi4.Schema.boolean().default(true).description("是否开启好感度升降显示"),
      prompt_0: import_koishi4.Schema.string().role("textarea").description("厌恶好感补充设定"),
      favorability_div_1: import_koishi4.Schema.number().default(15).description("厌恶-陌生分界线"),
      prompt_1: import_koishi4.Schema.string().role("textarea").description("陌生好感补充设定"),
      favorability_div_2: import_koishi4.Schema.number().default(150).description("陌生-朋友分界线"),
      prompt_2: import_koishi4.Schema.string().role("textarea").description("朋友好感补充设定"),
      favorability_div_3: import_koishi4.Schema.number().default(500).description("朋友-思慕分界线"),
      prompt_3: import_koishi4.Schema.string().role("textarea").description("思慕好感补充设定"),
      favorability_div_4: import_koishi4.Schema.number().default(1e3).description("思慕-恋慕分界线"),
      prompt_4: import_koishi4.Schema.string().role("textarea").description("恋慕好感补充设定")
    }).description("好感度设置"),
    import_koishi4.Schema.object({
      blockuser: import_koishi4.Schema.array(String).default([]).description("屏蔽的用户"),
      blockchannel: import_koishi4.Schema.array(String).default([]).description("屏蔽的频道")
    }).description("过滤器")
  ]);
})(Sat || (Sat = {}));

// src/index.tsx
var logger4 = new import_koishi5.Logger("satori-ai");
var SAT = class extends Sat {
  ...
  async handleSatCommand(session, prompt) {
    ...
    if (this.config.log_ask_response)
      logger4.info(`用户 ${session.username}：${prompt}`);
    ...
    if (this.config.log_ask_response)
      logger4.info(`Satori AI：${response.content}`);
  ...
  async handleAuxiliaryDialogue(session, prompt, response) {
    ...
    if (this.config.enable_auxiliary_LLM && !response.error && response.content) {
      ...
      if (result.error) {
        logger4.error(`辅助判断失败：${result.content}`);
      } else {
        logger4.info(`辅助判断：${result.content}`);
  ...
  // 对话次数检查
  async checkUserDialogueCount(session, user) {
  ...
  async generateResponse(session, prompt) {
    if (this.getChannelParallelCount(session) > this.config.max_parallel_count) {
      logger4.info(`频道 ${session.channelId} 并发数过高(${this.getChannelParallelCount(session)})，${session.username}等待中...`);
    }
    ...
    logger4.info(`频道 ${session.channelId} 处理：${session.userId},剩余${this.getChannelParallelCount(session)}并发`);
    const user = await ensureUserExists(this.ctx, session.userId, session.username);
  ...
  async buildSystemPrompt(session, prompt) {
    ...
    if (this.config.enable_favorability) {
      const user2 = await ensureUserExists(this.ctx, session.userId, session.username);
      systemPrompt += generateLevelPrompt(getFavorabilityLevel(user2, this.getFavorabilityConfig()), this.getFavorabilityConfig(), user2);
    }
    if (this.config.log_system_prompt)
      logger4.info(`系统提示：${systemPrompt}`);
    return systemPrompt;
  }
  // 处理回复
  async formatResponse(session, response, auxiliaryResult) {
    ...
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response).map((text) => import_koishi5.h.text(text));
  ...
  async handleMiddleware(session, prompt) {
    const user = await ensureUserExists(this.ctx, session.userId, session.username);
    if (this.performPreChecks(session, session.content))
      return null;
    if (await this.checkUserDialogueCount(session, user))
      return null;
