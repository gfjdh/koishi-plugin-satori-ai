...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    ...
    import_koishi5.Schema.object({
      ...
      value_of_output_mood: import_koishi5.Schema.number().default(10).description("输出触发屏蔽词每次扣除的心情,若当时心情大于0,此次不扣好感度"),
      ...
      pocket_money_cost: import_koishi5.Schema.number().default(10).description("消耗心情换p点的心情值")
...
`;
  ...
  async formatResponse(session, response, auxiliaryResult) {
    ...
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response, this.config.min_sentences_length, this.config.max_sentences_length).map((text) => import_koishi8.h.text(text));
