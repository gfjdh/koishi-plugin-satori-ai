...
((Sat2) => {
  Sat2.Config = import_koishi3.Schema.intersect([
    ...
    import_koishi3.Schema.object({
      ...
      enable_auxiliary_LLM: import_koishi3.Schema.boolean().default(false).description("是否使用辅助大模型判断好感度增减(量与上一项配置相关,不稳定慎用)"),
      offset_of_fafavorability: import_koishi3.Schema.number().default(3.5).description("辅助大模型好感度偏移量(越大越容易扣好感度)"),
      visible_favorability: import_koishi3.Schema.boolean().default(true).description("是否开启辅助大模型好感度升降显示"),
...
var SAT = class extends Sat {
  ...
  async handleAuxiliaryDialogue(session, prompt, response) {
    ...
    if (censor && this.config.visible_favorability)
      return "(好感↓↓)";
