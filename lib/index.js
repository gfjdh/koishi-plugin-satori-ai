...
发送"结束游戏"退出游戏`);
  ...
  async processInput(input) {
    if (input === "游戏规则")
      return await wrapInHTML(this.instuction);
    if (input === "技能列表")
      return await wrapInHTML(this.skillList);
    if (this.turnCount === 0)
...
`);
    ...
    return result ? wrapInHTML2(result) : null;
  }
};
var broadcastMessage = class {
  ...
  async sendMessage(session) {
    ...
    try {
      this.alreadySentChannels.add(channelId);
      if (this.img) {
        session.send(await wrapInHTML2(this.message));
      } else {
...
var randomPrompt = "根据群聊内最近的包括所有人的聊天记录，锐评一下群聊中的话题";
var puppeteer = null;
...
var SAT = class extends Sat {
  ...
  registerCommands(ctx) {
    ...
    ctx.command("sat.common_sense <text:text>", "添加常识").alias("添加常识").action(async ({ session }, prompt) => this.addCommonSense(session, prompt));
    ctx.command("sat.group_sense <text:text>", "添加群常识").alias("添加群常识").option("id", "-i <id:string>", { authority: 3 }).action(async ({ session, options }, prompt) => this.addGroupCommonSense(session, prompt, options.id));
