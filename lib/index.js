...
// src/index.tsx
var logger13 = new import_koishi14.Logger("satori-ai");
var puppeteer = null;
...
`;
  ...
  async formatResponse(session, response, auxiliaryResult) {
    ...
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response, this.config.min_sentences_length, this.config.max_sentences_length).map((text) => import_koishi14.h.text(text));
