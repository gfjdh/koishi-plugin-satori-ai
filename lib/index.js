...
var require_zh = __commonJS({
  "src/locales/zh.yml"(exports, module2) {
    module2.exports = { commands: { sat: { description: "AI聊天", messages: { "no-prompt": "你好。使用说明可以看我的空间哦。", tooManyEnglishLetters: "请不要用这么多英文......", err: "寄！{0}", tooLong: "你的话太多了。", content_tooLong: "这个问题有点复杂，你还是去问别人吧。", "duplicate-dialogue": "这个刚刚说过了吧......", block1: "我讨厌你！", block2: "我讨厌你们！" } }, "sat.clear": { description: "清空当前频道会话", messages: { clean: "已清空当前频道会话。" } }, "sat.common_sense": { description: "添加常识", messages: { "no-prompt": "你想让我知道什么？", succeed: "我知道了，{0}" } } } };
  }
});
...
var MemoryManager = class {
  ...
  // 更新短期记忆
  updateChannelMemory(session, prompt, response) {
  ...
  findBestMatches(entries, keywords, topN = 5) {
    return entries.map((entry) => ({ entry, ...this.calculateMatchScore(entry.content, keywords) })).filter(({ count }) => count > 1).sort((a, b) => b.score - a.score).slice(0, topN).map(({ entry }) => entry);
  }
  // 匹配度计算
...
var Sat = class extends import_koishi2.Service {
  static {
    __name(this, "Sat");
  }
  static inject = {
...
((Sat2) => {
  Sat2.Config = import_koishi2.Schema.intersect([
    import_koishi2.Schema.object({
      baseURL: import_koishi2.Schema.string().default("https://api.deepseek.com").description("请求地址"),
      ...
      appointModel: import_koishi2.Schema.string().default("deepseek-chat").description("模型"),
      prompt: import_koishi2.Schema.string().role("textarea").description("人格设定")
    }).description("基础设置"),
    import_koishi2.Schema.object({
      alias: import_koishi2.Schema.array(String).default(["ai"]).description("触发命令;别名"),
      authority: import_koishi2.Schema.number().role("slider").min(0).max(5).step(1).description("允许使用的最低权限").default(1),
      max_tokens: import_koishi2.Schema.number().description("最大请求长度（token）").default(100),
      content_max_tokens: import_koishi2.Schema.number().description("最大回答长度（token）").default(1024),
      message_max_length: import_koishi2.Schema.number().description("最大频道上下文长度（条数）").default(10),
      temperature: import_koishi2.Schema.number().role("slider").min(0).max(2).step(0.01).default(0.5).description("温度"),
      frequency_penalty: import_koishi2.Schema.number().default(0).description("频率惩罚"),
      presence_penalty: import_koishi2.Schema.number().default(0).description("存在惩罚"),
      maxRetryTimes: import_koishi2.Schema.number().default(30).description("报错后最大重试次数"),
      log_reasoning_content: import_koishi2.Schema.boolean().default(true).description("是否在日志中输出思维链（deepseek-r1）")
    }).description("请求设置"),
    import_koishi2.Schema.object({
      enable_self_memory: import_koishi2.Schema.boolean().default(true).description("是否启用模型自发言记忆（仅短期）"),
      dataDir: import_koishi2.Schema.string().default("./data/satori_ai").description("聊天记录保存位置（长期记忆）"),
      memory_block_words: import_koishi2.Schema.array(String).default(["好感"]).description("记忆屏蔽词"),
      remember_min_length: import_koishi2.Schema.number().description("触发保存到记忆的长度").default(20),
      common_topN: import_koishi2.Schema.number().default(5).description("常识记忆检索最大匹配数"),
      dailogues_topN: import_koishi2.Schema.number().default(5).description("对话记忆检索最大匹配数"),
      enable_fixed_dialogues: import_koishi2.Schema.boolean().default(false).description("是否启用固定对话（在dataDir中的fixed_dialogues.json修改）")
    }).description("记忆设置"),
    import_koishi2.Schema.object({
      ...
      random_min_tokens: import_koishi2.Schema.number().default(20).description("随机触发对话的最小长度"),
      randnum: import_koishi2.Schema.number().role("slider").min(0).max(1).step(0.01).default(0).description("在群聊中随机触发对话的概率，如需关闭可设置为 0"),
      sentences_divide: import_koishi2.Schema.boolean().default(true).description("是否分句发送"),
      time_interval: import_koishi2.Schema.number().default(1e3).description("每句话的时间间隔"),
      reply_pointing: import_koishi2.Schema.boolean().default(true).description("是否在与多人同时对话时显示回复指向")
    }).description("对话设置"),
    import_koishi2.Schema.object({
      enable_favorability: import_koishi2.Schema.boolean().default(false).description("是否开启好感度系统"),
      prompt_0: import_koishi2.Schema.string().role("textarea").description("厌恶好感补充设定"),
      favorability_div_1: import_koishi2.Schema.number().default(15).description("厌恶-陌生分界线"),
      prompt_1: import_koishi2.Schema.string().role("textarea").description("陌生好感补充设定"),
      favorability_div_2: import_koishi2.Schema.number().default(150).description("陌生-朋友分界线"),
      prompt_2: import_koishi2.Schema.string().role("textarea").description("朋友好感补充设定"),
      favorability_div_3: import_koishi2.Schema.number().default(500).description("朋友-思慕分界线"),
      prompt_3: import_koishi2.Schema.string().role("textarea").description("思慕好感补充设定"),
      favorability_div_4: import_koishi2.Schema.number().default(1e3).description("思慕-恋慕分界线"),
      prompt_4: import_koishi2.Schema.string().role("textarea").description("恋慕好感补充设定")
    }).description("好感度设置"),
    import_koishi2.Schema.object({
...
// src/index.tsx
var logger2 = new import_koishi3.Logger("satori-ai");
var SAT = class extends Sat {
  ...
  async handleSatCommand(session, prompt) {
    ...
    logger2.info(`用户 ${session.username}：${prompt}`);
    ...
    logger2.info(`Satori AI：${response.content}`);
  ...
  formatResponse(session, response) {
    ...
    if (this.config.sentences_divide) {
      return splitSentences(response).map((text) => import_koishi3.h.text(text));
    }
    return response;
  }
  // 清空会话
