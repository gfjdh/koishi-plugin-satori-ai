...
function filterResponse(prompt, words) {
  ...
  const filtered = parts.map((part) => {
    if (part.startsWith("（") && part.endsWith("）") || part.startsWith("(") && part.endsWith(")") || part.startsWith("[") && part.endsWith("]") || part.startsWith("【") && part.endsWith("】") || part.startsWith("<") && part.endsWith(">") || part.startsWith("《") && part.endsWith("》")) {
...
var goBangSingleGame = class extends abstractGameSingleGame {
  ...
  generateChat(Score) {
    if (this.lastScore < 5e6 && Score > 5e6 && this.level > 4) {
      return "我觉得你要输了哦~";
    }
