  ...
  "wasm/gobang.js"(exports, module2) {
    var Module2 = (() => {
      ...
      return async function(moduleArg = {}) {
        ...
        assert(typeof Module3["read"] == "undefined", "Module.read option was removed");
        ...
        assert(typeof Module3["setWindowTitle"] == "undefined", "Module.setWindowTitle option was removed (modify emscripten_set_window_title in JS)");
        ...
        function legacyModuleProp(prop, newName, incoming = true) {
          if (!Object.getOwnPropertyDescriptor(Module3, prop)) {
            Object.defineProperty(Module3, prop, { configurable: true, get() {
        ...
        function consumedModuleProp(prop) {
          if (!Object.getOwnPropertyDescriptor(Module3, prop)) {
            Object.defineProperty(Module3, prop, { configurable: true, set() {
        ...
        function unexportedRuntimeSymbol(sym) {
          if (!Object.getOwnPropertyDescriptor(Module3, sym)) {
            Object.defineProperty(Module3, sym, { configurable: true, get() {
        ...
        assert(!Module3["STACK_SIZE"], "STACK_SIZE can no longer be set at runtime.  Use -sSTACK_SIZE at link time");
        ...
        assert(!Module3["INITIAL_MEMORY"], "Detected runtime INITIAL_MEMORY setting.  Use -sIMPORTED_MEMORY to define wasmMemory dynamically");
        function preRun() {
          if (Module3["preRun"]) {
        ...
        var wasmImports = { _abort_js: __abort_js, _emscripten_runtime_keepalive_clear: __emscripten_runtime_keepalive_clear, _setitimer_js: __setitimer_js, emscripten_resize_heap: _emscripten_resize_heap, fd_close: _fd_close, fd_seek: _fd_seek, fd_write: _fd_write, proc_exit: _proc_exit };
        var wasmExports = await createWasm();
        var ___wasm_call_ctors = createExportWrapper("__wasm_call_ctors", 0);
        var _inspireSearch = Module3["_inspireSearch"] = createExportWrapper("inspireSearch", 4);
        var _wholeScore = Module3["_wholeScore"] = createExportWrapper("wholeScore", 2);
        var _fflush = createExportWrapper("fflush", 1);
        var __emscripten_timeout = createExportWrapper("_emscripten_timeout", 2);
        var _strerror = createExportWrapper("strerror", 1);
        var _malloc = Module3["_malloc"] = createExportWrapper("malloc", 1);
        var _free = Module3["_free"] = createExportWrapper("free", 1);
        ...
        for (const prop of Object.keys(Module3)) {
...
function filterResponse(prompt, words) {
  ...
  const filtered = parts.map((part) => {
    if (part.startsWith("（") && part.endsWith("）") || part.startsWith("(") && part.endsWith(")") || part.startsWith("[") && part.endsWith("]") || part.startsWith("【") && part.endsWith("】")) {
...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    ...
    import_koishi5.Schema.object({
      ...
      max_mood: import_koishi5.Schema.number().default(5).description("心情上限"),
      value_of_input_mood: import_koishi5.Schema.number().default(6).description("输入触发屏蔽词每次扣除的心情"),
      value_of_output_mood: import_koishi5.Schema.number().default(3).description("输出触发屏蔽词每次扣除的心情,若当时心情大于0,此次不扣好感度"),
      ...
      mood_div_2: import_koishi5.Schema.number().default(-5).description("烦躁-生气分界线"),
...
var goBangSingleGame = class extends abstractGameSingleGame {
  ...
  async processInput(str) {
    ...
    if (this.turnsCount > 5)
      setTimeout(() => {
        this.session.send(this.generateChat(aiMove.score)), 1e3;
      });
  ...
  // 负极大极小值搜索
  async alphaBeta(depth, alpha, beta, player, command, game) {
    let temp = command;
    ...
    for (let i = 0; i < length; i++) {
      this.place(steps[i], player, game);
      temp = await this.alphaBeta(depth, -beta, -alpha, 3 - player, steps[i], game);
      temp.score *= -1;
      this.place(steps[i], 0, game);
      if (temp.score >= beta) {
        temp.score = beta;
        return temp;
      }
  ...
  // 搜索入口
  async entrance(depth, alpha, beta, player, game) {
  ...
  async wasmInspireSearch(player) {
    const flatBoard = this.board.flat();
    wasmModule.HEAP32.set(flatBoard, wasmMemoryPool.boardPtr / 4);
    const length = await wasmModule._inspireSearch(
      wasmMemoryPool.scoreBoardPtr,
      // 直接使用预分配指针
      player,
      wasmMemoryPool.boardPtr,
      // 直接使用预分配指针
      inspireSearchLength
    );
  ...
  generateChat(Score) {
    if (this.lastScore < 5e6 && Score > 5e6) {
      return "我觉得你要输了哦~";
    }
