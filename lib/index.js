...
((Sat2) => {
  Sat2.Config = import_koishi4.Schema.intersect([
    ...
    import_koishi4.Schema.object({
      ...
      nick_name_list: import_koishi4.Schema.array(String).default(["昵称1"]).description("昵称列表"),
      nick_name_block_words: import_koishi4.Schema.array(String).default(["屏蔽词1"]).description("昵称屏蔽词(含有屏蔽词的消息不会触发昵称对话)"),
...
var SAT = class extends Sat {
  ...
  async checkDuplicateDialogue(session, prompt, recentDialogues, user) {
    if (!this.config.duplicateDialogueCheck)
      return null;
    if (session.content.includes(":poke"))
      return null;
  ...
  async processInput(session, prompt) {
    ...
    if (this.config.log_ask_response)
      logger4.info(`用户 ${session.username}：${processedPrompt}`);
    return censored;
  }
  // 生成回复
