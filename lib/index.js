...
((Sat2) => {
  Sat2.Config = import_koishi5.Schema.intersect([
    ...
    import_koishi5.Schema.object({
      ...
      reasoner_filter: import_koishi5.Schema.boolean().default(true).description("是否启用返回内容过滤,开启后在对话时会过滤掉含有过滤词的那一句,用于缓解思维链溢出问题"),
      reasoner_filter_word: import_koishi5.Schema.string().role("textarea").default("系统-提示-用户-设定-回复").description("返回内容过滤词，使用“-”分隔，再括号内且含有过滤词的那一句会被过滤，用于缓解思维链溢出问题"),
...
var SAT = class extends Sat {
  ...
  async buildSystemPrompt(session, prompt) {
    ...
    if (this.config.log_system_prompt)
      logger5.info(`系统提示：${systemPrompt}`);
    return systemPrompt;
  }
  // 处理回复
