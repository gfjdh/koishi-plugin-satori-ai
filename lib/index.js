...
var require_zh = __commonJS({
  "src/locales/zh.yml"(exports, module2) {
    module2.exports = { commands: { sat: { description: "AI聊天", messages: { censor: "不合规，请检查输入内容是否含有违禁词。", "no-prompt": "你好。", err: "寄！{0}", tooLong: "你的话太多了。", content_tooLong: "这个问题有点复杂，你还是去问别人吧。", "duplicate-dialogue": "这个刚刚说过了吧......", clean: "舒服了", block1: "我讨厌你！", block2: "我讨厌你们！" } }, "sat.clear": { description: "清空所有人的会话" }, "sat.common_sense": { description: "添加常识", messages: { "no-prompt": "你想让我知道什么？", succeed: "我知道了，{0}" } } } };
  }
});
...
((Sat2) => {
  Sat2.Config = import_koishi.Schema.intersect([
    ...
    import_koishi.Schema.object({
      ...
      temperature: import_koishi.Schema.number().role("slider").min(0).max(1).step(0.01).default(0).description("温度"),
...
var SAt = class extends Sat {
  ...
  async sat(session, prompt) {
    ...
    if (this.pluginConfig.enableContext) {
      let searchKeywordsInFile = function(filePath2, keywords2) {
        if (!fs2.existsSync(filePath2))
          return [];
        const fileContent = fs2.readFileSync(filePath2, "utf-8");
        const dialogues2 = JSON.parse(fileContent);
        const keywordRegex = new RegExp(keywords2.join("|"), "gi");
        const chineseRegex = /[\u4e00-\u9fa5]/;
        let matchCounts = dialogues2.map((dialogue, index) => {
          ...
          if (matches) {
            matches.forEach((match) => {
              ...
              count += chineseCount * 3 + englishCount;
            });
          }
          ...
          let totalCount = totalChineseCount * 3 + totalEnglishCount;
        ...
        const sortedMatches2 = filteredMatchCounts.sort((a, b) => b.ratio - a.ratio);
        return sortedMatches2;
      }, appendTopMatches = function(dialogues2, sortedMatches2, topN, prefix) {
        const topMatches = sortedMatches2.slice(0, topN);
        const personalityContent = `
${prefix}{
` + topMatches.map((item) => dialogues2[item.index].content).join("\n") + "\n}";
      ...
      const charactersToRemove = ["的", "一", "是", "了", "什", "么", "我", "谁", "不", "人", "在", "他", "有", "这", "个", "上", "们", "来", "到", "时", "大", "地", "为", "子", "中", "你", "说", "生", "国", "年", "着", "就", "那", "和", "要", "她", "出", "也", "得", "里", "后", "自", "以", "会"];
      ...
      const commonSenseContent = JSON.parse(fs2.readFileSync(commonSenseFilePath, "utf-8"));
