...
var UserPortraitManager = class {
  ...
  async getDialogues(user) {
    ...
    if (!fs5.existsSync(memoryPath))
      return [];
    const MemoryContent = await JSON.parse(fs5.readFileSync(memoryPath, "utf-8"));
  ...
  // 生成提示词模板
  buildMessage(level, history, existingPortrait) {
    return [{
      role: "user",
...
·不使用markdown等标记语言，直接书写即可`
  ...
  async generatePortrait(session, user, apiClient) {
    ...
    logger5.info(`${messages[0].content}`);
    try {
...
// src/index.tsx
var logger6 = new import_koishi7.Logger("satori-ai");
var SAT = class extends Sat {
  ...
  async handleAuxiliaryDialogue(session, prompt, response) {
    ...
    if (this.config.enable_auxiliary_LLM && !response.error && response.content) {
      ...
      if (result.error) {
        logger6.error(`辅助判断失败：${result.content}`);
      } else {
        logger6.info(`辅助判断：${result.content}`);
  ...
  async processInput(session, prompt) {
    ...
    if (this.config.log_ask_response)
      logger6.info(`用户 ${session.username}：${processedPrompt}`);
    return censored;
  }
  // 生成回复
  async generateResponse(session, prompt) {
    if (this.getChannelParallelCount(session) > this.config.max_parallel_count) {
      logger6.info(`频道 ${session.channelId} 并发数过高(${this.getChannelParallelCount(session)})，${session.username}等待中...`);
    }
    ...
    logger6.info(`频道 ${session.channelId} 处理：${session.userId},剩余${this.getChannelParallelCount(session)}并发`);
    const user = await getUser(this.ctx, session.userId);
    const response = await this.apiClient.chat(user, await messages);
    if (this.config.log_ask_response)
      logger6.info(`Satori AI：${response.content}`);
    return response;
  }
  // 构建消息
...
${payload}`);
  ...
  async formatResponse(session, response, auxiliaryResult) {
    ...
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response).map((text) => import_koishi7.h.text(text));
