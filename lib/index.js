...
var goBangSingleGame = class extends abstractGameSingleGame {
  ...
  startGame = () => {
    ...
    logger8.info(`level: ${this.level}, playerFlag: ${this.playerFlag}`);
  ...
  async processInput(str) {
    ...
    logger8.info(`AI 落子坐标: ${aiX} ${aiY}，得分: ${score}`);
...
// src/game.ts
var logger9 = new import_koishi10.Logger("satori-game");
var Game = class {
  static {
    __name(this, "Game");
  }
  ...
  async startGame(session, gameName, args) {
    ...
    logger9.info(`游戏${gameName}已开始于${session.channelId}`);
  }
  // 结束当前游戏
  async endGame(session) {
    ...
    this.channelGames.delete(session.channelId);
    logger9.info(`${session.channelId}的游戏已结束`);
  }
  // 选择并启动具体游戏实例
...
// src/index.tsx
var logger10 = new import_koishi11.Logger("satori-ai");
var SAT = class extends Sat {
  ...
  async handleAuxiliaryDialogue(session, prompt, response) {
    ...
    if (this.config.enable_auxiliary_LLM && !response.error && response.content) {
      ...
      if (result.error) {
        logger10.error(`辅助判断失败：${result.content}`);
      } else {
        logger10.info(`辅助判断：${result.content}`);
  ...
  async processInput(session, prompt) {
    ...
    if (this.config.log_ask_response)
      logger10.info(`用户 ${session.username}：${processedPrompt}`);
    return censored;
  }
  // 生成回复
  async generateResponse(session, prompt) {
    if (this.getChannelParallelCount(session) > this.config.max_parallel_count) {
      logger10.info(`频道 ${session.channelId} 并发数过高(${this.getChannelParallelCount(session)})，${session.username}等待中...`);
    }
    ...
    logger10.info(`频道 ${session.channelId} 处理：${session.userId},剩余${this.getChannelParallelCount(session)}并发`);
    const user = await getUser(this.ctx, session.userId);
  ...
  async getChatResponse(user, messages) {
    ...
    if (this.config.log_ask_response) {
      if (this.config.enable_favorability && this.config.enable_mood)
        logger10.info(`Satori AI：（心情值：${this.moodManager.getMoodValue(user.userid)}）${response.content}`);
      else
        logger10.info(`Satori AI：${response.content}`);
    }
...
`;
  ...
  async formatResponse(session, response, auxiliaryResult) {
    ...
    if (this.config.sentences_divide && response.length > 10) {
      const sentences = splitSentences(response, this.config.min_sentences_length, this.config.max_sentences_length).map((text) => import_koishi11.h.text(text));
