...
// src/middleware.ts
function createMiddleware(ctx, sat, config, memoryManager) {
  return async (session, next) => {
    if (session.content.length <= config.max_tokens) {
      let censored = session.content;
      if (ctx.censor)
        censored = await ctx.censor.transform(session.content, session);
...
async function handleRandomTrigger(SAT2, session, config) {
  ...
  return SAT2.handleMiddleware(session, "根据最近群友的发言，请你随意聊一下");
}
