...
var require_zh = __commonJS({
  "src/locales/zh.yml"(exports, module2) {
    module2.exports = { commands: { sat: { description: "AI聊天", messages: { "no-prompt": "你好。使用说明可以看我的空间哦。", tooManyEnglishLetters: "请不要用这么多英文......", err: "寄！{0}", tooLong: "你的话太多了。", content_tooLong: "这个问题有点复杂，你还是去问别人吧。", "duplicate-dialogue": "这个刚刚说过了吧......", "no-response": "我不知道你在说什么……", block1: "我讨厌你！", block2: "我讨厌你们！" } }, "sat.clear": { description: "清空当前频道会话", messages: { clean: "已清空当前频道会话。" } }, "sat.common_sense": { description: "添加常识", messages: { "no-prompt": "你想让我知道什么？", succeed: "我知道了，{0}" } } } };
  }
});
...
async function handleContentCheck(ctx, content, userid, config) {
...
((Sat2) => {
  Sat2.Config = import_koishi3.Schema.intersect([
    ...
    import_koishi3.Schema.object({
      ...
      max_tokens: import_koishi3.Schema.number().description("最大请求长度（token）").default(100),
      content_max_tokens: import_koishi3.Schema.number().description("最大回答长度（token）").default(1024),
    ...
    import_koishi3.Schema.object({
      enable_favorability: import_koishi3.Schema.boolean().default(false).description("是否开启好感度系统(每次对话+1好感度)"),
      censor_favorability: import_koishi3.Schema.boolean().default(false).description("是否开启好感度审查(通过屏蔽词扣除好感)"),
      value_of_favorability: import_koishi3.Schema.number().default(15).description("屏蔽词每次扣除的好感度"),
      enable_auxiliary_LLM: import_koishi3.Schema.boolean().default(false).description("是否使用辅助模型判断(通过大模型进行额外的好感度增减,量与value_of_favorability相关)"),
...
var SAT = class extends Sat {
  ...
  // 处理辅助判断
  async handleAuxiliaryDialogue(session, prompt, responseConent) {
    const user = await ensureUserExists(this.ctx, session.userId, session.username);
    const regex = /\*\*/g;
  ...
  async generateResponse(session, prompt) {
    while (this.getChannelParallelCount(session) > this.config.max_parallel_count) {
      logger3.info(`频道 ${session.channelId} 并发数过高，${session.username}等待中...`);
  ...
  // 格式化回复
  formatResponse(session, response, auxiliaryResult) {
    this.updateChannelParallelCount(session, -1);
    if (!response)
